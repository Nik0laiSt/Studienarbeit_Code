...
class FlowTransformer:
    ...
    def build_model(self, prefix:str=None):
        ...
        # By leaving training=None, we allow Keras to handle the training state during fit/predict/evaluate
        training = None
        ...
        m_x = self.input_encoding.apply(m_inputs, prefix, training=training)

        # in case the classification head needs to add tokens at this stage
        m_x = self.classification_head.apply_before_transformer(m_x, prefix, training=training)

        m_x = self.sequential_model.apply(m_x, prefix, training=training)
        m_x = self.classification_head.apply(m_x, prefix, training=training)
        ...
        for layer_i, layer_size in enumerate(self.parameters.mlp_layer_sizes):
            ...
            m_x = Dropout(self.parameters.mlp_dropout)(m_x, training=training) if self.parameters.mlp_dropout > 0 else m_x
    ...
    def evaluate(self, m:keras.Model, batch_size, early_stopping_patience:int=5, epochs:int=100, steps_per_epoch:int=128):
        ...
        def run_evaluation(epoch):
            ...
            pred_probabilities = m.predict(eval_featurewise_X, verbose=True).reshape(-1)
            
            # 4. Vorhersagen inspizieren
            print("\n--- Inspektion der Vorhersagen (erste 10) ---")
            for i in range(10):
                print(f"Wahrer Wert: {eval_y[i]}, Vorhergesagte Wahrscheinlichkeit: {pred_probabilities[i]:.4f}")
            print("--------------------------------------------\n")
            ...
            # 3. Konfusionsmatrix und weitere Metriken
            print("--- Konfusionsmatrix & Metriken ---")
            ...
            try:
                ...
                plt.title(f'Konfusionsmatrix (Epoche {epoch})')
            ...
            print(classification_report(eval_y, pred_y, target_names=['Benign', 'Attack']))
            print("-------------------------------------\n")

            # 5. Schwellenwert visualisieren
            print("--- Verteilung der Vorhersage-Wahrscheinlichkeiten ---")
            try:
                plt.figure(figsize=(10, 6))
                plt.hist(pred_probabilities[eval_y == 0], bins=50, label='Benign (Wahr)', alpha=0.7, density=True)
                plt.hist(pred_probabilities[eval_y == 1], bins=50, label='Attack (Wahr)', alpha=0.7, density=True)
                plt.axvline(threshold, color='red', linestyle='--', label=f'Schwellenwert = {threshold}')
                plt.title(f'Verteilung der Wahrscheinlichkeiten (Epoche {epoch})')
                plt.xlabel('Vorhergesagte Wahrscheinlichkeit für \'Attack\'')
                plt.ylabel('Dichte')
                plt.legend()
                plt.show()
            except Exception as e:
